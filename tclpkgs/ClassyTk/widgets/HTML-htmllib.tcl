...
source [file join $::Classy::dir html_library-0.3 html_library.tcl]
...
Classy::HTML method init {args} {
	...
	$w tag bind link <<Action>> {%W _followlink [%W linkat %x %y]}
...
#doc {HTML options -hyperlinkcommand} option {-hyperlinkcommand hyperlinkCommand Command} descr {
#}
Classy::HTML addoption -hyperlinkcommand {hyperlinkCommand Command {}} {
}
...
Classy::HTML method geturl {url {query {}}} {
	private $object options currentquery html
	...
	if ![info exists history(reload)] {
		# don't reload html when going to a different fragment
		if {"$query" == ""} {
			if {"$currentbase" == "$base"} {
...
Classy::HTML method _setimage {handle url} {
	...
	if {"[info commands $image]" == ""} {
		...
		switch $protocol {
			...
			file {
				if {[regexp {/[A-Z]:} $file]} {set file [string range $file 1 end]}
...
Classy::HTML method _followlink {url} {
	private $object options
	if {![string length $options(-hyperlinkcommand)]} {
		$object geturl $url
	} else {
		uplevel #0 $options(-hyperlinkcommand) $url
	}
